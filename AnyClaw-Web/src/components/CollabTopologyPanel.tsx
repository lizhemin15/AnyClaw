...
export default function CollabTopologyPanel({
  ...
  const onNodeClickSelect = useCallback(
    ...
    [topologyReady, pendingSlug, toggleEdge]
  )
  ...
  interactionRef.current = { topologyReady, agentsLen: agents.length, toggleEdge, onNodeClickSelect }

  const onNodePointerDown = (slug: string, e: React.PointerEvent) => {
    if (e.button !== 0) return
    const { topologyReady: tr, agentsLen } = interactionRef.current
    if (!tr || agentsLen < 2) return
    e.preventDefault()
    ...
    const onUp = (ev: PointerEvent) => {
      ...
      const { topologyReady: ok, agentsLen: n, toggleEdge: te, onNodeClickSelect: clickSel } = interactionRef.current
      if (!p || !ok || n < 2) return
  ...
  return (
    <div className={`rounded-2xl border border-violet-200/80 bg-gradient-to-b from-violet-50/40 to-white p-4 sm:p-5 space-y-3 ${className}`}>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex flex-wrap items-center gap-2 min-h-[1.5rem]">
            {pendingSlug && topologyReady && agents.length > 1 && (
              <>
          ...
          {dirty && topologyReady && agents.length > 1 && (
            <button
              type="button"
      ...
      {err && agents.length > 0 && <p className="text-red-600 text-xs">{err}</p>}
    </div>
  )
}
