...
func (d *DB) CreateHost(h *Host) error {
	_, err := d.Exec(
		`INSERT INTO hosts (id, name, addr, ssh_port, ssh_user, ssh_key, docker_image, enabled, status)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		h.ID, h.Name, h.Addr, h.SSHPort, h.SSHUser, h.SSHKey, h.DockerImage,
...
func (d *DB) GetHost(id string) (*Host, error) {
	...
	var enabled int
	err := d.QueryRow(
		`SELECT id, name, addr, ssh_port, ssh_user, ssh_key, docker_image, enabled, status, last_check_at, created_at
		 FROM hosts WHERE id = ?`,
		id,
	).Scan(&h.ID, &h.Name, &h.Addr, &h.SSHPort, &h.SSHUser, &sshKey, &dockerImage,
...
func (d *DB) ListEnabledHosts() ([]*Host, error) {
	rows, err := d.Query(
		`SELECT id, name, addr, ssh_port, ssh_user, ssh_key, docker_image, enabled, status
	...
	for rows.Next() {
		...
		var dockerImage sql.NullString
		var enabled int
		if err := rows.Scan(&h.ID, &h.Name, &h.Addr, &h.SSHPort, &h.SSHUser, &sshKey, &dockerImage, &enabled, &h.Status); err != nil {
			return nil, err
		}
		h.Enabled = true
		if sshKey.Valid {
...
func (d *DB) UpdateHost(h *Host) error {
	_, err := d.Exec(
		`UPDATE hosts SET name=?, addr=?, ssh_port=?, ssh_user=?, ssh_key=?, docker_image=?, enabled=?
		 WHERE id = ?`,
		h.Name, h.Addr, h.SSHPort, h.SSHUser, h.SSHKey, h.DockerImage, enabledInt(h.Enabled), h.ID,
	)
	return err
}
