...
import (
	...
	"github.com/anyclaw/anyclaw-server/pkg/providers/api_proxy"
)

// CreateProvider creates a provider based on the configuration.
// When AnyClawBridge is enabled (container mode), returns API proxy provider;
// otherwise uses model_list. Returns provider, model ID, and error.
func CreateProvider(cfg *config.Config) (LLMProvider, string, error) {
	model := cfg.Agents.Defaults.GetModelName()

	// Container mode: use API proxy (admin holds keys, container only has instance token)
	if cfg.Channels.AnyClawBridge.IsEnabled() {
		provider := api_proxy.NewProvider(
			cfg.Channels.AnyClawBridge.APIURL,
			cfg.Channels.AnyClawBridge.InstanceID,
			cfg.Channels.AnyClawBridge.Token,
		)
		// Model comes from config; strip protocol prefix if present
