...
func (s *appState) modelMenu() tview.Primitive {
	...
	items = append(items,
		...
		MenuItem{
			...
			Action: func() {
				s.addModel(
					AnyClawconfig.ModelConfig{ModelName: "new-model", Model: "openai/gpt-5.2"},
				)
				s.push(
...
func (s *appState) addModel(model AnyClawconfig.ModelConfig) {
...
func refreshModelMenu(menu *Menu, currentModel string, models []AnyClawconfig.ModelConfig) {
...
func isModelValid(model AnyClawconfig.ModelConfig) bool {
...
func (s *appState) testModel(model *AnyClawconfig.ModelConfig) {
	if model == nil {
		return
	}
