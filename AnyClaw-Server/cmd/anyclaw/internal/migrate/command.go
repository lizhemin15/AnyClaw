...
import (
	...
	"github.com/anyclaw/anyclaw-server/pkg/migrate"
)
...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to AnyClaw",
		Args:  cobra.NoArgs,
		Example: `  AnyClaw migrate
  AnyClaw migrate --from openclaw
  AnyClaw migrate --dry-run
  AnyClaw migrate --refresh
  AnyClaw migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.anyclaw)")
